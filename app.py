...
import json
# ── 2026 Championship order (default grid) ─────────────────────
default_grid_2026 = {
    1:  {'driver': 'ANT', 'team': 'Mercedes',        'points': 179},
    2:  {'driver': 'RUS', 'team': 'Mercedes',        'points': 154},
    3:  {'driver': 'HAM', 'team': 'Ferrari',         'points': 147},
    4:  {'driver': 'LEC', 'team': 'Ferrari',         'points': 108},
    5:  {'driver': 'NOR', 'team': 'McLaren',         'points': 97},
    6:  {'driver': 'PIA', 'team': 'McLaren',         'points': 82},
    7:  {'driver': 'VER', 'team': 'Red Bull Racing', 'points': 76},
    8:  {'driver': 'HAD', 'team': 'Red Bull Racing', 'points': 52},
    9:  {'driver': 'GAS', 'team': 'Alpine',          'points': 42},
    10: {'driver': 'LAW', 'team': 'Racing Bulls',    'points': 37},
    11: {'driver': 'LIN', 'team': 'Racing Bulls',    'points': 19},
    12: {'driver': 'BEA', 'team': 'Haas F1 Team',   'points': 18},
    13: {'driver': 'COL', 'team': 'Alpine',          'points': 18},
    14: {'driver': 'BOR', 'team': 'Kick Sauber',     'points': 6},
    15: {'driver': 'SAI', 'team': 'Williams',        'points': 6},
    16: {'driver': 'ALB', 'team': 'Williams',        'points': 5},
    17: {'driver': 'OCO', 'team': 'Haas F1 Team',   'points': 3},
    18: {'driver': 'ALO', 'team': 'Aston Martin',   'points': 1},
    19: {'driver': 'HUL', 'team': 'Kick Sauber',    'points': 0},
    20: {'driver': 'BOT', 'team': 'Caddilac',       'points': 0},
    21: {'driver': 'PER', 'team': 'Caddilac',       'points': 0},
    22: {'driver': 'STR', 'team': 'Aston Martin',   'points': 0},
}
...
raw_teams = encoders['team'].classes_.tolist()
teams = sorted(set(TEAM_DISPLAY_ALIAS.get(t, t) for t in raw_teams))
...
tab1, tab2, tab3, tab4, tab5, tab6, tab7, tab8 = st.tabs([
    "🏁 Prediction", "📊 Season Analysis", "🏆 Constructor Stats", "🔍 Model Insights",
    "⚔️ Head-to-Head", "🧠 SHAP Explainer", "🎮 Race Simulation", "📈 Season Projection"
])
# ════════════════════════════════════════════════
# TAB 1 — PREDICTION
# ════════════════════════════════════════════════
with tab1:
    if predict_btn:
        ...
        try:
            ...
            team_enc = encoders['team'].transform([TEAM_ENCODE_ALIAS.get(selected_team, selected_team)])[0]
        ...
        if selected_team == 'Cadillac':
            st.caption("ℹ️ Cadillac has no historical F1 data — this prediction uses a midfield team as a rough proxy, not a learned pattern.")
        ...
        with col2:
            st.metric("Win Probability", f"{round(probability*100, 1)}%")
...
with tab2:
    ...
    selected_year = st.selectbox("Select Season", [2019, 2020, 2021, 2022, 2023, 2024, 2025], key='season_year')
    ...
    races_2024 = ['Bahrain Grand Prix', 'Saudi Arabian Grand Prix','Australian Grand Prix', 'Japanese Grand Prix','Chinese Grand Prix', 'Miami Grand Prix','Emilia Romagna Grand Prix', 'Monaco Grand Prix','Canadian Grand Prix', 'Spanish Grand Prix','Austrian Grand Prix', 'British Grand Prix','Hungarian Grand Prix', 'Belgian Grand Prix',
...
with tab3:
    ...
    selected_year2 = st.selectbox(
        "Select Season", [2019, 2020, 2021, 2022, 2023,2024,2025], key='constructor_year'
    )
...
with tab5:
    ...
    if st.button("⚔️ Compare", width='stretch'):
        ...
        team_a_enc = encoders['team'].transform([TEAM_ENCODE_ALIAS.get(team_a, team_a)])[0]
        ...
        team_b_enc = encoders['team'].transform([TEAM_ENCODE_ALIAS.get(team_b, team_b)])[0]
        ...
        with col_r1:
            st.metric(f"{driver_a} Win Probability", round(prob_a*100, 1))
        with col_r2:
            st.metric(f"{driver_b} Win Probability", round(prob_b*100, 1))
...
with tab6:
    ...
    **SHAP (SHapley Additive exPlanations)** reveals how much 
    ...
    # ── Key insights ──────────────────────────────────────────
# ── Key insights — dynamically generated from live SHAP data ──
    st.markdown("### 💡 Key Insights from SHAP Analysis")

    ranked = sorted(shap_data.items(), key=lambda x: x[1], reverse=True)
    top_feature, top_val = ranked[0]
    second_feature, second_val = ranked[1]
    weakest_feature, weakest_val = ranked[-1]
    ...
    with col1:
        ...
        **🏎️ {top_feature} Leads**
        ...
        {top_feature} is the most influential feature ({top_val:.2f}) — 
        it drives race outcome more than any other factor in the model.
     """)

        st.info(f"""
        **📈 Runner-Up: {second_feature}**
    
        {second_feature} ({second_val:.2f}) ranks 2nd — a meaningful 
        secondary signal the model relies on.
        """)
    ...
    with col2:
        ...
        **🏟️ Circuit DNA**
        ...
        Circuit ({shap_data.get('Circuit', 0):.2f}) reflects track-specific 
        patterns — certain drivers perform consistently better at certain venues.
        """)

        st.info(f"""
        **📉 Least Influential: {weakest_feature}**
    
        {weakest_feature} ({weakest_val:.2f}) has minimal impact — 
        the model leans on it least among all ten features.
        """)
    ...
    if st.button("🧠 Explain This Prediction", key='shap_btn'):
        ...
        team_enc = encoders['team'].transform([TEAM_ENCODE_ALIAS.get(shap_team, shap_team)])[0]
        ...
        if prediction == 1:
            st.success(f"🏆 {shap_driver} predicted to WIN! "
                      f"({round(probability*100, 1)}% probability)")
        else:
            st.error(f"❌ {shap_driver} NOT predicted to win "
                    f"({round(probability*100, 1)}% win probability)")
...
with tab7:
    ...
    on 2026 championship standings. Reorder drivers to simulate 
    different qualifying scenarios.
    """)
    # Add this at the top of tab7, before race conditions
    sim_season = st.radio(
        "Select Grid Season",
        ["2026 (Current)", "2023 (Training Data)"],
        horizontal=True,
        key='sim_season'
    )

# Use appropriate grid based on selection
    if sim_season == "2026 (Current)":
        active_grid = default_grid_2026
        unsupported = [d for d in [info['driver'] for info in active_grid.values()] if d not in drivers]
        if unsupported:
            st.info(f"⚠️ {', '.join(unsupported)} has no historical F1 race data in this "
                    f"model's training set (2019-2025) and cannot be predicted. This will "
                    f"resolve automatically once real 2026 race results are available to retrain on.")
        else:
            st.success("✅ All 2026 grid drivers are supported by the model.")

# replacing default_grid_2026[pos] with active_grid[pos] in the loop
    ...
    # Splitting 22 drivers into two columns of half each
    for pos in range(1, len(active_grid)+1):
        driver_info = active_grid[pos]
        ...
        grid_data.append({
            ...
            'Team': selected_team,
            'StandingPoints': default_grid_2026[pos]['points']
        })
    ...
    if st.button("🚀 Simulate Race!", use_container_width=True):
        ...
        for driver_data in grid_data:
            try:
                ...
                team_enc = encoders['team'].transform([TEAM_ENCODE_ALIAS.get(driver_data['Team'], driver_data['Team'])])[0]
        ...
        if 'Cadillac' in results_df['Team'].values:
            st.caption("ℹ️ Cadillac drivers have no historical F1 data — their predictions use a midfield proxy, not learned team performance.")

        
        # ── Predicted winner announcement ──────────────────────
        winner = results_df.iloc[0]
        st.success(f"🏆 Predicted Race Winner: **{winner['Driver']}** "
                  f"({round(winner['WinProbability'], 1)}% win probability)")
        ...
        with pod1:
            st.metric("🥇 1st", results_df.iloc[0]['Driver'],
                     f"{round(results_df.iloc[0]['WinProbability'], 1)}%")
        with pod2:
            st.metric("🥈 2nd", results_df.iloc[1]['Driver'],
                     f"{round(results_df.iloc[1]['WinProbability'], 1)}%")
        with pod3:
            st.metric("🥉 3rd", results_df.iloc[2]['Driver'],
                     f"{round(results_df.iloc[2]['WinProbability'], 1)}%")
        ...
        fig_sim = go.Figure(go.Bar(
            ...
            orientation='h',
            marker_color=colors,
            text=[f"{round(p, 1)}%" for p in results_df['WinProbability']],
...
with tab8:
    st.subheader("📈 Season Points Projection")
    st.markdown("""
    Simulates the rest of the 2026 season using Monte Carlo sampling. For each 
    remaining race, the model estimates win probabilities from each driver's 
    recent form, then race outcomes are sampled probabilistically (not just 
    "favorite always wins") and repeated many times to build a realistic range 
    of final standings — not a single guess.
    """)

    remaining_races = st.multiselect(
        "Select remaining races on the calendar",
        circuits,
        default=[]
    )

    n_sims = st.slider("Number of simulations", 100, 3000, 500, step=100)
    points_system = {1:25, 2:18, 3:15, 4:12, 5:10, 6:8, 7:6, 8:4, 9:2, 10:1}

    if st.button("🎲 Run Season Simulation") and remaining_races:
        current_standings = {info['driver']: info['points'] for info in default_grid_2026.values()}
        driver_teams = {info['driver']: info['team'] for info in default_grid_2026.values()}

        # Recent form fallback for rookies / unsupported drivers
        recent_form = {}
        for drv in current_standings:
            drv_rows = df[(df['Abbreviation'] == drv) & (df['Year'] == 2025)]
            if len(drv_rows) > 0:
                recent_form[drv] = {
                    'grid': drv_rows['GridPosition'].mean(),
                    'quali': drv_rows['QualiPosition'].mean()
                }
            else:
                recent_form[drv] = {'grid': 12, 'quali': 12}  # neutral midfield guess

        sim_results = {drv: [] for drv in current_standings}
        progress = st.progress(0)

        for sim in range(n_sims):
            season_points = dict(current_standings)

            for race in remaining_races:
                try:
                    circuit_enc = encoders['circuit'].transform([race])[0]
                except ValueError:
                    continue
                ...
                for drv in current_standings:
                    if drv not in drivers:
                        win_probs[drv] = 0.01  # no model data — small floor, not zero
                        continue
                    try:
                        driver_enc = encoders['driver'].transform([drv])[0]
                        team_enc = encoders['team'].transform([driver_teams[drv]])[0]
                    except ValueError:
                        win_probs[drv] = 0.01
                        continue

                    grid_pos = max(1, int(np.random.normal(recent_form[drv]['grid'], 2)))
                    quali_pos = max(1, int(np.random.normal(recent_form[drv]['quali'], 2)))

                    input_data = np.array([[
                        driver_enc, team_enc, circuit_enc,
                        grid_pos, quali_pos, 0, 25, 35,
                        season_points.get(drv, 0), 0
                    ]])
                    prob = model.predict_proba(scaler.transform(input_data))[0][1]
                    win_probs[drv] = max(prob, 0.001)

                # Plackett-Luce: sample a full top-10 order weighted by win probability
                pool = list(win_probs.keys())
                w = np.array([win_probs[d] for d in pool])
                order = []
                for _ in range(min(10, len(pool))):
                    w_norm = w / w.sum()
                    idx = np.random.choice(len(pool), p=w_norm)
                    order.append(pool.pop(idx))
                    w = np.delete(w, idx)

                for pos, drv in enumerate(order, start=1):
                    season_points[drv] = season_points.get(drv, 0) + points_system.get(pos, 0)

            for drv in current_standings:
                sim_results[drv].append(season_points[drv])

            if sim % max(1, n_sims // 20) == 0:
                progress.progress(min(sim / n_sims, 1.0))

        progress.progress(1.0)

        summary = [{
            'Driver': drv,
            'Current Points': current_standings[drv],
            'Projected Mean': round(np.mean(vals), 1),
            'P10': round(np.percentile(vals, 10), 1),
            'P90': round(np.percentile(vals, 90), 1)
        } for drv, vals in sim_results.items()]

        summary_df = pd.DataFrame(summary).sort_values('Projected Mean', ascending=False).reset_index(drop=True)

        all_drivers = list(current_standings.keys())
        sim_matrix = np.array([sim_results[d] for d in all_drivers])
        winner_idx = np.argmax(sim_matrix, axis=0)
        title_counts = pd.Series(winner_idx).value_counts()
        title_pct = {all_drivers[i]: round(100 * c / n_sims, 1) for i, c in title_counts.items()}
        summary_df['Title Win %'] = summary_df['Driver'].map(title_pct).fillna(0.0)
        ...
        for _, row in summary_df.iterrows():
            fig_proj.add_trace(go.Bar(
                name=row['Driver'], x=[row['Driver']], y=[row['Projected Mean']],
                error_y=dict(type='data', symmetric=False,
                             array=[row['P90'] - row['Projected Mean']],
                             arrayminus=[row['Projected Mean'] - row['P10']]),
                showlegend=False
            ))
        fig_proj.update_layout(
            title="Projected Final Points (10th–90th percentile range)",
